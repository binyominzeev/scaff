...
export function parseProjectSpec(rawContent) {
  ...
  const seedMatch = rest.match(/```json\n# seed-data\n([\s\S]*?)\n```/);
  let seedData = null;
  if (seedMatch) {
    try {
      seedData = JSON.parse(seedMatch[1].trim());
    } catch (err) {
      throw new Error(`Hibás JSON a # seed-data blokkban: ${err.message}`);
    }
  }
 ...
 *   "AppPreference.singleton()"
 *   "Person.create()"
 *   "Person.update({id})"
 *   "AppPreference.updateSingleton()"
 *   "Person.delete({id})"
 */
export function parseOperation(expr) {
  const m = expr.trim().match(/^(\w+)\.(findMany|findUnique|singleton|create|updateSingleton|update|delete)\((.*)\)$/);
  if (!m) return null;
  ...
  if (op === "singleton" || op === "updateSingleton") {
    return { model, op, filterField: null, paramName: null };
  }
...
/**
 * Guardrail against the most common LLM authoring mistake: writing a descriptive,
 * accented and/or multi-word label (e.g. "havi bevétel") instead of the real Prisma
 * field name (e.g. "income") as the first token of an `[Input: ...]`/`[Select: ...]`
 * line. The strict per-kind regex silently skips such lines, which then makes the
 * screen look like it has zero fields and fails much later with a confusing error.
 * This scans for every raw `[Kind: ...]` occurrence and fails fast, right at parse
 * time, pointing at the exact offending line and the likely cause.
 */
function assertBracketTokensParse(body, route, kindLabel, strictPattern) {
  const rawTokens = body.match(new RegExp(`\\[${kindLabel}:[^\\]]*\\]`, "g")) || [];
  const matchedCount = [...body.matchAll(strictPattern)].length;
  if (rawTokens.length === matchedCount) return;

  const offending = rawTokens.find((raw) => !new RegExp(`^${strictPattern.source}$`).test(raw));
  throw new Error(
    `"${route}": érvénytelen [${kindLabel}: ...] sor: ${offending || rawTokens[rawTokens.length - 1]}\n` +
      `A mezőnévnek (a "${kindLabel}:" utáni első szónak) a valódi Prisma modell-mező nevével kell ` +
      `egyeznie: kizárólag ASCII betűk/számok/alsóvonás, szóköz és ékezet nélkül (pl. "income", nem ` +
      `"havi bevétel").`
  );
}
...
export function parseUiScreens(uiScreensBlock) {
  ...
  return screenChunks.map((chunk) => {
    ...
    assertBracketTokensParse(body, route, "Input", /\[Input:\s*(\w+)\s*\(([^)]*)\)(?:\s*placeholder="([^"]*)")?\]/g);
    assertBracketTokensParse(body, route, "Select", /\[Select:\s*(\w+)\s*;\s*options=([^;]+);\s*value=(\w+);\s*label=(\w+)(?:;\s*required)?\]/g);
    ...
    for (const m of body.matchAll(/\[Select:\s*(\w+)\s*;\s*options=([^;]+);\s*value=(\w+);\s*label=(\w+)(?:;\s*required)?\]/g)) {
      const [, name, optionsExpr, valueField, labelField] = m;
      elements.push({
        kind: "select",
        name,
        options: parseOperation(optionsExpr.trim()),
        valueField,
        labelField,
    ...
    for (const m of body.matchAll(/\[Link:\s*([^\]]*?)\s*->\s*([^\]]+)\]/g)) {
      ...
      elements.push({ kind: "link", label: label.trim(), target: target.trim(), _pos: m.index });
    }
    ...
    for (const m of body.matchAll(/\[List:\s*rows=([^\]]+)\]/g)) {
      elements.push({ kind: "list", operation: parseOperation(m[1].trim()), _pos: m.index });
    }

    for (const m of body.matchAll(/\[InteractiveList:\s*rows=([^;]+);\s*primary=(\w+);\s*secondary=(\w+)(?:;\s*reveal="([^"]+)")?(?:;\s*next="([^"]+)")?\]/g)) {
      const [, rowsExpr, primaryField, secondaryField, revealLabel, nextLabel] = m;
      elements.push({
        kind: "interactive-list",
        operation: parseOperation(rowsExpr.trim()),
        primaryField,
        secondaryField,
        revealLabel: revealLabel || "Megjelenítés",
        nextLabel: nextLabel || "Következő",
        _pos: m.index,
      });
    }

    for (const m of body.matchAll(/\[Data:\s*([^\]]+)\]/g)) {
      elements.push({ kind: "data", operation: parseOperation(m[1].trim()), _pos: m.index });
    }

    for (const m of body.matchAll(/\[Heading:\s*([^\]]+)\]/g)) {
      elements.push({ kind: "heading", value: m[1].trim(), _pos: m.index });
    }

    for (const m of body.matchAll(/\[Text:\s*([^\]]+)\]/g)) {
      elements.push({ kind: "text", value: m[1].trim(), _pos: m.index });
    }

    // Elements are discovered kind-by-kind above, so re-sort them into the order they
    // actually appear in the source wireframe (needed so e.g. a [Heading:] can be placed
    // right before a specific Table/List instead of always floating to a fixed spot).
    elements.sort((a, b) => a._pos - b._pos);
    for (const el of elements) delete el._pos;
