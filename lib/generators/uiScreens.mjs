import { writeFile, modelToResourceName, humanizeRouteSegment, humanizeSlug } from "./common.mjs";
...
/** Whether screenRoutes contains an actual "/<segment>/[param]"-shaped dynamic detail screen. */
function hasDynamicDetailRoute(detailRoute, screenRoutes) {
  for (const route of screenRoutes) {
    const parts = route.split("/").filter(Boolean);
    if (parts.length === 2 && parts[0] === detailRoute && /^\[.+\]$/.test(parts[1])) return true;
  }
  return false;
}
...
export function generateNextjsUiScreens(outDir, spec, warnings) {
  ...
  const projectName = spec.frontMatter?.project?.name;
  const screenRoutes = new Set(spec.screens.map((s) => s.route));
  const designLook = resolveDesignLook(spec.frontMatter?.design?.look);
  const archetype = resolveLayoutArchetype(spec.frontMatter?.design?.components);
  for (const screen of spec.screens) {
    ...
    if (formButton) {
      generateFormScreen(outDir, screen, formButton, inputEls, selectEls, warnings, authEnabled, screenRoutes, projectName, designLook);
    } else {
      generateReadScreen(outDir, screen, { primary, hasRecord, deleteButton }, warnings, authEnabled, projectName, designLook, archetype);
    }
  }
}

function generateFormScreen(outDir, screen, formButton, inputEls, selectEls, warnings, authEnabled, screenRoutes, projectName, designLook) {
  ...
  if (op === "create") {
    ...
    const heading = interpolate(
      screen.heading || `Új ${model}`,
      { params: screen.params, hasRecord: false },
      warnings,
      screen.route
    );
...
export function ${clientFormName}({ ${optionProps} }: { ${selectEls.map((select) => `${select.name}Options: Array<Record<string, unknown>>`).join("; ")} }) {
  ...
  return (
    <main className="${designLook.container.narrow}">
      <h1 className="${designLook.heading.formH1}">{\`${heading}\`}</h1>
      {error && <p className="mt-4 text-sm ${designLook.text.danger}">{error}</p>}
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
${clientFields}
        <div className="flex items-center gap-4"><Button type="submit" disabled={pending}>{pending ? "Mentés…" : "Mentés"}</Button><Link href="${redirectTarget}" className="${designLook.text.link}">Mégse</Link></div>
      </form>
    </main>
  );
}
...
${selectFields}
        <div className="flex items-center gap-4">
          ...
          <Link href="${redirectTarget}" className="${designLook.text.link}">
            Mégse
          </Link>
        </div>
      </form>
...
`;
  ...
  const fallbackReturnTarget = resolveCreateRedirectTarget(screen.route, screenRoutes);
  // A per-record detail page like "/cards/[id]" only exists for real route-param updates —
  // singleton updates (e.g. "/siman") have no such page, so cancel/success must not assume one.
  const cancelTarget = op !== "updateSingleton" && hasDynamicDetailRoute(detailRoute, screenRoutes)
    ? `/${detailRoute}/${"${recordId}"}`
  ...
  // A route-derived id (or updateSingleton) is a "real" identifier — no matching row is a genuine
  // 404. An auth-derived currentUserId identifier isn't: it just means this user hasn't saved a
  // preference yet, so the form must still render (with empty defaults) instead of erroring out.
  const implicitCurrentUser = op !== "updateSingleton" && !isRouteParam;
  const recordFetchLine = op === "updateSingleton"
    ? ""
    : currentUserIdLine + `  const record = await prisma.${varModel}.findUnique({ where: { id: ${idExpr} } });\n`;
  const notFoundGuard = implicitCurrentUser
    ? ""
    : `\n  if (!record) {\n    return <p className="${designLook.container.page} ${designLook.palette.textMuted}">Nem található.</p>;\n  }\n`;

  const serverPage = `import { prisma } from "@/lib/prisma";
${authImport}import { ${formComponentName} } from "./form";

${paramsPreamble}${optionFetches ? optionFetches + "\n" : ""}${recordFetchLine}${notFoundGuard}
  return <${formComponentName} initialData={record} recordId={${idExpr}}${selectEls.length ? ` ${selectEls.map((select) => `${select.name}Options={${select.name}Options}`).join(" ")}` : ""} />;
}
`;
...
export function ${formComponentName}({ initialData, recordId${selectEls.length ? `, ${updateOptionProps}` : ""} }: { initialData: InitialData | null; recordId: string${selectEls.length ? `; ${updateOptionTypes}` : ""} }) {
  const router = useRouter();
  const [values, setValues] = useState({ ${initialValues} });
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.push("${fallbackReturnTarget}");
  }
...
${selectFields}
        <div className="flex items-center gap-4">
          ...
          <Link href={\`${cancelTarget}\`} className="${designLook.text.link}">
            Mégse
          </Link>
        </div>
      </form>
...
function generateReadScreen(outDir, screen, ctx, warnings, authEnabled, projectName, designLook, archetype) {
  const { primary, hasRecord, deleteButton } = ctx;
  ...
  // If the spec didn't provide a title line, fall back to the project name on the home
  // screen and to a humanized route segment everywhere else, so no page ever ships titleless.
  const fallbackHeading = screen.route === "/" ? (projectName ? humanizeSlug(projectName) : null) : humanizeRouteSegment(screen.route);
  const headingText = interpolate(
    screen.heading || fallbackHeading || "",
    { params: screen.params, hasRecord, fallbackVar },
    warnings,
    screen.route
  );
  ...
  const bodyParts = [];
  if (headingText) {
    bodyParts.push(`      <h1 className="${designLook.heading.h1}">{\`${headingText}\`}</h1>`);
  }

  // "stat-summary-band" archetype: a small counts band above the first data section, purely additive.
  if (archetype.showStatBand && (tableEls.length > 0 || listEls.length > 0)) {
    const statSources = [
      ...tableEls.map((el, i) => ({ label: el.operation.model, varName: tableVarNames[i] })),
      ...listEls.map((el, i) => ({ label: el.operation.model, varName: listVarNames[i] })),
    ];
    bodyParts.push(`      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-${Math.max(statSources.length, 1)}">`);
    for (const s of statSources) {
      bodyParts.push(
        `        <div className="${designLook.card.wrapper}">`,
        `          <p className="${designLook.text.muted}">${s.label}</p>`,
        `          <p className="text-2xl font-bold ${designLook.palette.textPrimary}">{${s.varName}.length}</p>`,
        `        </div>`
      );
    }
    bodyParts.push(`      </div>`);
  ...
  // Render the remaining elements in the order they were authored (not grouped by kind),
  // so a [Heading:] token ends up right above the section it was meant to introduce.
  let tableIndex = 0;
  let listIndex = 0;
  let interactiveIndex = 0;
  for (const el of screen.elements) {
    if (el.kind === "heading") {
      const interpolated = interpolate(el.value, { params: screen.params, hasRecord, fallbackVar }, warnings, screen.route);
      bodyParts.push(`      <h2 className="${designLook.heading.h2}">{\`${interpolated}\`}</h2>`);
    } else if (el.kind === "text") {
      const interpolated = interpolate(el.value, { params: screen.params, hasRecord, fallbackVar }, warnings, screen.route);
      bodyParts.push(`      <p className="${designLook.text.body}">{\`${interpolated}\`}</p>`);
    } else if (el.kind === "table") {
      const varName = tableVarNames[tableIndex++];
      const rowLinkArrow = buildRowLinkArrow(el.rowLink);
      const columnsLiteral = JSON.stringify(el.columns);
      bodyParts.push(
        `      <div className="mt-6">`,
    ...
    } else if (el.kind === "interactive-list") {
      const varName = interactiveVarNames[interactiveIndex++];
      bodyParts.push(
        `      <InteractiveList rows={${varName}} primaryField="${el.primaryField}" secondaryField="${el.secondaryField}" revealLabel="${el.revealLabel}" nextLabel="${el.nextLabel}" />`
      );
    } else if (el.kind === "link") {
      const href = interpolate(el.target, { params: screen.params, hasRecord }, warnings, screen.route);
      bodyParts.push(
        `      <Link href={\`${href}\`} className="mt-4 inline-block ${designLook.text.link}">`,
        `        ${el.label}`,
        `      </Link>`
      );
    } else if (el.kind === "button" && el.isNavigation) {
      const href = interpolate(el.action, { params: screen.params, hasRecord }, warnings, screen.route);
      bodyParts.push(
        `      <div className="mt-4">`,
        `        <Button href={\`${href}\`}>${el.label}</Button>`,
        `      </div>`
      );
    }
  }
  ...
  // findUnique keyed by a real route param with no matching row is a genuine 404. findUnique keyed
  // by the auth-derived currentUserId (e.g. a per-user preference) just means this user hasn't saved
  // one yet, so the rest of the page (other tables/lists) must still render instead of erroring out.
  const primaryIsImplicitCurrentUser = Boolean(primary && primary.paramName && !screen.params.includes(primary.paramName));
  const pageContainerClass = archetype.containerWidth || designLook.container.page;
  const recordGuard = hasRecord && !primaryIsImplicitCurrentUser
    ? `\n  if (!record) {\n    return <p className="${pageContainerClass} ${designLook.palette.textMuted}">Nem található.</p>;\n  }\n`
    : "";
