...
export function generateNextjsUiScreens(outDir, spec, warnings) {
  ...
  for (const screen of spec.screens) {
    ...
    if (formButton) {
      generateFormScreen(outDir, screen, formButton, inputEls, selectEls, warnings, authEnabled, screenRoutes, projectName);
    } else {
      generateReadScreen(outDir, screen, { primary, hasRecord, deleteButton }, warnings, authEnabled, projectName);
    }
  }
}

function generateFormScreen(outDir, screen, formButton, inputEls, selectEls, warnings, authEnabled, screenRoutes, projectName) {
...
${selectFields}
        <div className="flex items-center gap-4">
          ...
          <Link href={\`${cancelTarget}\`} className="text-sm text-slate-600 hover:underline">
            Mégse
          </Link>
        </div>
      </form>
...
function generateReadScreen(outDir, screen, ctx, warnings, authEnabled, projectName) {
  ...
  const bodyParts = [];
  if (headingText) {
    bodyParts.push(`      <h1 className="text-3xl font-bold tracking-tight">{\`${headingText}\`}</h1>`);
  }
  ...
  for (const el of screen.elements) {
    if (el.kind === "heading") {
      ...
      bodyParts.push(`      <h2 className="mt-8 text-lg font-semibold text-slate-900">{\`${interpolated}\`}</h2>`);
