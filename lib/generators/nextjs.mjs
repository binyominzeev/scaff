...
import { writePocketIdOidcAuth } from "./oidcAuth.mjs";
import { resolveDesignLook, resolveLayoutArchetype } from "../designCatalog.mjs";

function buildPrismaSeed(seedData, prismaBlock) {
  ...
  const idFieldsByModel = extractModelIdFields(prismaBlock);
  ...
  for (const [model, modelRecords] of Object.entries(seedData)) {
    ...
    const idField = idFieldsByModel[model] || "id";
    for (const [index, record] of modelRecords.entries()) {
      ...
      const data = { ...record };
      delete data._alias;
      if (idField === "id" && !data.id) data.id = `seed_${alias}`;
      if (data[idField] === undefined) {
        throw new Error(`A # seed-data blokkban a ${model}[${index}] (${alias}) rekordnak meg kell adnia a(z) "${idField}" mezőt, mert ez a modell elsődleges kulcsa.`);
      }
      records.push({ alias, model, data, idField });
    }
  }
...
async function main() {
  for (const entry of records) {
    const model = (prisma as unknown as Record<string, { upsert(args: unknown): Promise<Record<string, unknown>> }>)[entry.model];
    ...
    const result = await model.upsert({ where: { [entry.idField]: data[entry.idField] }, update: data, create: data });
    created.set(entry.alias, { id: result[entry.idField] as string });
  }
}
...
export function generateNextjsProject(outDir, spec, warnings = []) {
  ...
  const designLook = resolveDesignLook(frontMatter.design?.look);
  ...
  } else if (auth.enabled && auth.provider === "pocket-id-oidc") {
    dependencies = mergeDeps(dependencies, { jose: "^6.0.11", "server-only": "^0.0.1" });
    writePocketIdOidcAuth(outDir);
    warnings.push(
      `⚠️  Pocket ID / OIDC auth generálva. Regisztráld a Pocket ID adminban a redirect URI-t (<origin>/auth/callback, pl. fejlesztésben http://localhost:3000/auth/callback), és töltsd ki a .env-ben: NEXT_PUBLIC_OIDC_ISSUER, NEXT_PUBLIC_OIDC_CLIENT_ID, OIDC_CLIENT_SECRET, OIDC_REDIRECT_URI.`
    );
  }
  ...
  const authEnabled = auth.enabled && auth.provider === "pocket-id-oidc";
  const authImport = authEnabled ? `import { AuthProvider } from "@/components/auth-provider";\n` : "";
  // Nav itself renders <AuthControls /> (see uiKit.mjs), so only import it here when there's no Nav.
  const authControlsImport = authEnabled && !navItems.length ? `import { AuthControls } from "@/components/ui/auth-controls";\n` : "";
  const bodyOpen = authEnabled ? "<AuthProvider>\n        " : "";
  const bodyClose = authEnabled ? "\n        </AuthProvider>" : "";
  const standaloneAuthControls = authEnabled && !navItems.length ? `<div className="flex justify-end p-4">\n          <AuthControls />\n        </div>\n        ` : "";

  writeFile(
    outDir,
    "app/layout.tsx",
    navItems.length
      ? `import type { Metadata } from "next";\nimport "./globals.css";\n${authImport}import { Nav } from "@/components/ui/nav";\n\nexport const metadata: Metadata = {\n  title: "${project.name}",\n  description: "${project.one_liner.trim().replace(/"/g, "'")}",\n};\n\nexport default function RootLayout({\n  children,\n}: Readonly<{ children: React.ReactNode }>) {\n  return (\n    <html lang="hu">\n      <body>\n        ${bodyOpen}<Nav />\n        {children}${bodyClose}\n      </body>\n    </html>\n  );\n}\n`
      : `import type { Metadata } from "next";\nimport "./globals.css";\n${authImport}${authControlsImport}\nexport const metadata: Metadata = {\n  title: "${project.name}",\n  description: "${project.one_liner.trim().replace(/"/g, "'")}",\n};\n\nexport default function RootLayout({\n  children,\n}: Readonly<{ children: React.ReactNode }>) {\n  return (\n    <html lang="hu">\n      <body>\n        ${bodyOpen}${standaloneAuthControls}{children}${bodyClose}\n      </body>\n    </html>\n  );\n}\n`
  );
  ...
  if (!screenRoutes.has("/")) {
    writeFile(
      outDir,
      "app/page.tsx",
      `export default function Home() {\n  return (\n    <main className="${designLook.container.page}">\n      <h1 className="${designLook.heading.h1}">${humanizeSlug(project.name)}</h1>\n      <p className="${designLook.text.body}">${project.one_liner.trim().replace(/`/g, "'")}</p>\n    </main>\n  );\n}\n`
    );
  }
  ...
  for (const p of pages) {
    ...
    writeFile(
      outDir,
      routePath,
      `export default function ${componentName}() {\n  return (\n    <main className="${designLook.container.page}">\n      <h1 className="${designLook.heading.h1}">${stubTitle}</h1>\n      <p className="${designLook.text.body}">${(p.description || "TODO: implement").replace(/`/g, "'")}</p>\n    </main>\n  );\n}\n`
    );
  }
    ...
    if (api.enabled) {
      ...
      for (const model of models) {
        ...
        writeFile(
          ...
          `import { NextResponse } from "next/server";\nimport { prisma } from "@/lib/prisma";\n\nfunction prismaError(error: unknown) {\n  const code = typeof error === "object" && error && "code" in error ? String((error as { code: unknown }).code) : "";\n  const status = code === "P2025" ? 404 : code === "P2002" || code === "P2003" ? 400 : 500;\n  return NextResponse.json({ error: code ? \`Adatbázis hiba (\${code}).\` : "A művelet nem sikerült." }, { status });\n}\n\ntype Params = { params: Promise<{ id: string }> };\n\nexport async function GET(request: Request, { params }: Params) {\n  try {\n    const { id } = await params;\n    const item = await prisma.${varModel}.findUnique({ where: { id } });\n    if (!item) return NextResponse.json({ error: "Not found" }, { status: 404 });\n    return NextResponse.json(item);\n  } catch (error) {\n    return prismaError(error);\n  }\n}\n\nexport async function PATCH(request: Request, { params }: Params) {\n  try {\n    const { id } = await params;\n    const body = await request.json();\n    const updated = await prisma.${varModel}.upsert({ where: { id }, create: { ...body, id }, update: body });\n    return NextResponse.json(updated);\n  } catch (error) {\n    return prismaError(error);\n  }\n}\n\nexport async function DELETE(request: Request, { params }: Params) {\n  try {\n    const { id } = await params;\n    await prisma.${varModel}.delete({ where: { id } });\n    return NextResponse.json({ success: true });\n  } catch (error) {\n    return prismaError(error);\n  }\n}\n`
        );
      }
    }

    writeFile(outDir, "prisma/seed.ts", buildPrismaSeed(seedData, prismaBlock));
  }

  // ---- v0.2: UI kit + generated screens from the "ui-screens" block ----
  if (screens.length) {
    const archetype = resolveLayoutArchetype(frontMatter.design?.components);
    if (archetype.requestedId && archetype.requestedId !== archetype.id) {
      warnings.push(
        `⚠️  design.components: "${archetype.requestedId}" még nincs implementálva a motorban — a scaffold "${archetype.id}" rendereléssel generálta le. Lásd README.md "Design looks & layout archetypes".`
      );
    }
    writeUiKit(outDir, { framework: "nextjs-app-router", navItems, projectName: project.name, authEnabled, designLook: frontMatter.design?.look, designComponents: frontMatter.design?.components });
  ...
  const envLines = [];
  if (database.engine === "postgres") {
    envLines.push(`DATABASE_URL="postgresql://postgres:postgres@localhost:5432/${project.name.replace(/-/g, "_")}"`);
  }
  if (authEnabled) {
    envLines.push(
  ...
  if (envLines.length) {
    ...
    writeFile(outDir, ".env.example", envContent);
    // real .env with dev-friendly defaults; OIDC_CLIENT_SECRET/NEXT_PUBLIC_OIDC_CLIENT_ID still need to be filled in by hand
    writeFile(outDir, ".env", envContent);
  }
