 ...
 */
export function writeUiKit(outDir, { framework, navItems, projectName, authEnabled }) {
...
`
  ...
  if (navItems && navItems.length) {
    ...
    writeFile(
      ...
      `${linkImport}\n${authControlsImport}\nconst links = [\n${linksArray}\n];\n\nexport function Nav() {\n  return (\n    <nav className="border-b border-slate-200 bg-white">\n      <div className="mx-auto flex max-w-5xl items-center gap-6 px-4 py-3">\n        <Link ${linkProp}="/" className="text-base font-semibold tracking-tight text-slate-900">\n          ${projectName ? humanizeSlug(projectName) : ""}\n        </Link>\n        <div className="flex flex-1 items-center gap-6 text-sm font-medium text-slate-600">\n          {links.map((l) => (\n            <Link key={l.href} ${linkProp}={l.href} className="hover:text-slate-900">\n              {l.label}\n            </Link>\n          ))}\n        </div>${authControlsJsx}\n      </div>\n    </nav>\n  );\n}\n`
    );
  }
